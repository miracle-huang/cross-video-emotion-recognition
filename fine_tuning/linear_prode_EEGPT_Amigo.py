...
class LitEEGPTCausal(pl.LightningModule):
    def __init__(self, exp_name, load_path="checkpoint/eegpt_mcae_58chs_4s_large4E.ckpt"):
        super().__init__()    
        # init model
        ...
        # 通道标识符，用于指定要使用的通道，写死为 AMIGO 数据集的 14 个通道
    ...
    def on_test_epoch_end(self) -> None:
        ...
        with open(f"result/test_results_video_amigo_{self.exp_name}.txt", "w") as f:
...
if __name__ == "__main__":
    ...
    mat_dir = "datasets/downstream/amigo/windowsize_10"
    mat_files = [os.path.join(mat_dir, f) for f in os.listdir(mat_dir) if f.endswith(".mat")]
    mat_files = [f.replace("\\", "/") for f in mat_files]

    video_list = list(range(1, 17))  # AMIGO 数据集的视频列表
    amigo_half_valence = config.AMIGO_half_valence_low + config.AMIGO_half_valence_high
    amigo_half_arousal = config.AMIGO_half_arousal_low + config.AMIGO_half_arousal_high
    amigo_half = {
        "valence_labels": amigo_half_valence,
        "arousal_labels": amigo_half_arousal
    }
    for key, half_video_list in amigo_half.items():
        for video_id in video_list:
            ...
            # 初始化模型
            model = LitEEGPTCausal(exp_name=key, load_path="checkpoint/eegpt_mcae_58chs_4s_large4E.ckpt")
            ...
            # 创建训练日志记录器
            train_logger = pl_loggers.CSVLogger('./logs/', name=f"EEGPT_amigo_half_{label_key}_video_train", version=f"video{video_id}")
            tb_logger = pl_loggers.TensorBoardLogger('./logs/', name=f"EEGPT_amigo_half_{label_key}_video_tb", version=f"video{video_id}")
            ...
            # 创建测试日志记录器
            test_logger = pl_loggers.CSVLogger('./logs/', name=f"EEGPT_amigo_half_{label_key}_video_test", version=f"video{video_id}")
            ...
            # 保存训练好的模型权重
            torch.save(model.state_dict(), f"result/liteeegpt_causal_video_amigo_half_{video_id}.pth")
