...
def data_processing_amigo():
    # 只使用16个短视频
    all_video_list_de = [[] for _ in range(16)]
    all_video_list_psd = [[] for _ in range(16)]
        ...
        for filename in filenames:
            if filename.endswith('.mat'):
                ...
                for video_index in range(16):
                    arrays_to_stack_de, arrays_to_stack_psd = process_data_in_one_video(signal_data, video_index)
    ...
    for video_index in range(16):
        ...
        arousal_labels = np.repeat(config.AMIGO_video_arousal_labels[int(video_index)], video_array_de.shape[0])
        valence_labels = np.repeat(config.AMIGO_video_valence_labels[int(video_index)], video_array_psd.shape[0])
        processed_data_de = {
            'data': video_array_de,
